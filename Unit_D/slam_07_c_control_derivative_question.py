...
class ExtendedKalmanFilter:
    ...
    def dg_dcontrol(state, control, w):
        ...
        if r != l:
            ...
            temp_dl = w * r / (r - l) ** 2
            temp_dr = -w * l / (r - l) ** 2
            temp_s = sin(theta_new) - sin(theta)
            temp_c = -cos(theta_new) + cos(theta)
            temp_div = (r + l) / (2. * (r - l))

            dg1_l = (temp_dl * temp_s) - (temp_div * cos(theta_new))
            dg2_l = (temp_dl * temp_c) - (temp_div * sin(theta_new))

            dg1_r = (temp_dr * temp_s) + (temp_div * cos(theta_new))
            dg2_r = (temp_dr * temp_c) + (temp_div * sin(theta_new))
            ...
            dg1_l = (cos(theta) + (l / w) * sin(theta)) / 2.
            dg2_l = (sin(theta) - (l / w) * cos(theta)) / 2.

            dg1_r = ((-l / w) * sin(theta) + cos(theta)) / 2.
            dg2_r = ((l / w) * cos(theta) + sin(theta)) / 2.
